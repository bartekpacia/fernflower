...
public class ClassWriter {
  ...
  /**
   * Returns true if <code>mt</code> is an autogenerated constructor that is most likely
  ...
  private static boolean isDefaultRecordConstructor(StructClass cl, StructMethod mt, TextBuffer code) {
    ...
    final var componentsDescriptor = components.stream()
      .map(StructField::getDescriptor)
      .collect(Collectors.joining());

    final var name = mt.getName();

    final var isConstructor = name.equals(CodeConstants.INIT_NAME);
    if (!isConstructor) {
      return false;
    }

    var constructorDescriptor = mt.getDescriptor().replaceFirst("[(]", "").replaceFirst("[)]", "");
    ...
    for (final var component : components) {
      ...
      expectedDefaultCode.append("this.").append(fieldName).append("=").append(fieldName).append(";");
    }

    final var actualDefaultCode = code.toString().replaceAll("\\s+","");
    return expectedDefaultCode.toString().equals(actualDefaultCode);
  }

  /**
   * Returns true if <code>mt</code> is an autogenerated field accessor that is most likely
   * not present in the original source code.
   */
        ...
        if (root != null && !methodWrapper.decompiledWithErrors) { // check for existence
          try {
            ...
            hideMethod = code.length() == 0 &&
              ...
              isDefaultRecordConstructor(cl, mt, code) ||
